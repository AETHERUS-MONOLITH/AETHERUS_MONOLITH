...
const expectedTrack328PipelineStatuses = [
...
const expectedTrack328OrderedStages = [
  'packet_validation',
  'candidate_intake',
...
const expectedTrack328FixtureIds = [
...
const expectedTrack329ReportFixtureIds = [
  'valid_current_supported_report',
  'valid_non_pinned_candidate_eligible_report',
  'invalid_non_pinned_marked_supported',
  'invalid_missing_supported_vault_commit',
  'invalid_wrong_supported_vault_commit',
  'invalid_stage_order',
  'invalid_blocked_without_reasons',
  'invalid_missing_metadata_only_assertion',
  'invalid_claims_runtime_multi_vault_support',
...
const expectedTrack331FailureInjectionReportFixtureIds = [
  'valid_failure_injection_report',
  'invalid_missing_supported_vault_commit',
  'invalid_wrong_supported_vault_commit',
  'invalid_injection_count_mismatch',
  'invalid_pass_fail_count_mismatch',
  'invalid_unblocked_injected_failure',
  'invalid_missing_blocking_reason',
  'invalid_allows_non_pinned_support',
  'invalid_missing_metadata_only_assertion',
  'invalid_claims_alternate_vault_execution',
  'invalid_claims_active_vault_switch',
  'invalid_claims_runtime_multi_vault_support',
  'invalid_claims_public_runtime_wiring',
  'invalid_treats_track3_runs_as_persistent_ledger',
...
function validateTrack328CompatibilityPipelineFixtures(suite, evidencePacketContract) {
  const file = 'data/nexus-vault-compatibility-pipeline-fixtures.v1.json';
  const category = 'Track 3.28 Vault compatibility pipeline fixtures';

  requirePath(suite, ['metadata'], file, category);
  requirePath(suite, ['supported_vault_commit'], file, category);
  requirePath(suite, ['ordered_stages'], file, category);
  requirePath(suite, ['allowed_pipeline_statuses'], file, category);
  requirePath(suite, ['required_fixture_ids'], file, category);
  requirePath(suite, ['fixture_policy'], file, category);
  requirePath(suite, ['fixtures'], file, category);
  ...
  if (metadata.track_phase !== '3.28') {
    addFailure(file, category, `metadata.track_phase must be 3.28, found "${metadata.track_phase}"`);
  }
  if (metadata.status !== 'local_compatibility_pipeline_stub_fixtures') {
    addFailure(file, category, `metadata.status must be local_compatibility_pipeline_stub_fixtures, found "${metadata.status}"`);
  }
  [
    ...expectedTrack322BoundaryFalseFlags,
    'alternate_vault_execution',
  ...
  validateExpectedMembers(file, category, suite.ordered_stages, expectedTrack328OrderedStages, 'ordered_stages');
  expectedTrack328OrderedStages.forEach((stage, index) => {
    if ((suite.ordered_stages || [])[index] !== stage) {
      addFailure(file, category, `ordered_stages[${index}] must be ${stage}`);
    }
  });
  validateExpectedMembers(file, category, suite.allowed_pipeline_statuses, expectedTrack328PipelineStatuses, 'allowed_pipeline_statuses');
  validateExpectedMembers(file, category, suite.required_fixture_ids, expectedTrack328FixtureIds, 'required_fixture_ids');
  ...
  [
    ...
    'must_not_switch_active_vault',
    'must_not_create_runtime_multi_vault_support',
    'packet_validation_precedes_intake',
    'intake_precedes_compatibility_evaluation',
  ...
  const allowedStatuses = new Set(suite.allowed_pipeline_statuses || []);
  const requiredFields = evidencePacketContract && evidencePacketContract.required_packet_fields || expectedTrack326PacketFields;
  const fixtures = Array.isArray(suite.fixtures) ? suite.fixtures : [];
  const fixtureIds = new Set();
  ...
  fixtures.forEach((fixture, index) => {
    ...
    fixtureIds.add(fixture.fixture_id);
    requirePath(fixture, ['fixture_id'], file, category);
    requirePath(fixture, ['compatibility_evaluation'], file, category);
    requirePath(fixture, ['expected_pipeline_status'], file, category);
    requirePath(fixture, ['expected_supported'], file, category);

    if (!allowedStatuses.has(fixture.expected_pipeline_status)) {
      addFailure(file, category, `${label} uses unsupported expected_pipeline_status ${fixture.expected_pipeline_status}`);
    }
    if (fixture.expected_blocking_stage !== null && !expectedTrack328OrderedStages.includes(fixture.expected_blocking_stage)) {
      addFailure(file, category, `${label} expected_blocking_stage must be null or an ordered stage`);
    }
    if (typeof fixture.expected_supported !== 'boolean') {
      addFailure(file, category, `${label} expected_supported must be boolean`);
    }
    ...
    if (evaluation.executes_vault_code !== false) {
      addFailure(file, category, `${label} compatibility_evaluation.executes_vault_code must be false`);
    }

    if (fixture.fixture_id === 'missing_packet_pipeline') {
      if (fixture.packet !== null) {
        addFailure(file, category, 'missing_packet_pipeline fixture must use packet: null');
      }
      if (fixture.expected_pipeline_status !== 'blocked_at_packet_validation') {
        addFailure(file, category, 'missing_packet_pipeline must be blocked_at_packet_validation');
      }
      return;
    }
    ...
    const packet = fixture.packet;
    if (fixture.fixture_id !== 'invalid_packet_pipeline') {
      requiredFields.forEach(field => {
        if (packet[field] === undefined || packet[field] === null) {
          addFailure(file, category, `${label} packet missing ${field}`);
        }
    ...
    if (packet.candidate_vault_commit !== suite.supported_vault_commit && fixture.expected_pipeline_status === 'current_supported_pass') {
      addFailure(file, category, `${label} non-pinned candidate must not end current_supported_pass`);
    }
    if (packet.candidate_vault_commit !== suite.supported_vault_commit && fixture.expected_supported === true) {
      addFailure(file, category, `${label} non-pinned candidate must not be marked supported`);
    }
    if (fixture.expected_pipeline_status === 'candidate_pipeline_eligible' && fixture.expected_supported !== false) {
      addFailure(file, category, `${label} candidate_pipeline_eligible must not be support acceptance`);
    }
    if (fixture.expected_blocking_stage && (!Array.isArray(packet.blocking_reasons) || !packet.blocking_reasons.length)) {
      addFailure(file, category, `${label} blocked pipeline fixture must include blocking_reasons`);
    }
  });

  expectedTrack328FixtureIds.forEach(fixtureId => {
    if (!fixtureIds.has(fixtureId)) {
      addFailure(file, category, `Missing fixture ${fixtureId}`);
    }
  });
...
function validateTrack328CompatibilityPipelineScript() {
  const file = 'scripts/run-nexus-vault-compatibility-pipeline-stub.mjs';
  if (!existsSync(path.join(repoRoot, file))) {
    addFailure(file, 'Track 3.28 Vault compatibility pipeline', 'Compatibility pipeline script is missing');
    return;
  }

  const text = readText(file);
  [
    ['fixture input', /data\/nexus-vault-compatibility-pipeline-fixtures\.v1\.json/],
    ['evidence packet contract input', /data\/nexus-vault-compatibility-evidence-packet-contract\.v1\.json/],
    ['intake fixture input', /data\/nexus-vault-candidate-intake-fixtures\.v1\.json/],
    ['compatibility fixture input', /data\/nexus-vault-compatibility-evaluation-fixtures\.v1\.json/],
    ['ignored report output', /\.track3-runs\/latest-nexus-vault-compatibility-pipeline-report\.json/],
    ['current supported pass status', /current_supported_pass/],
    ['candidate pipeline eligible status', /candidate_pipeline_eligible/],
    ['blocked packet validation status', /blocked_at_packet_validation/],
    ['blocked intake status', /blocked_at_intake/],
    ['blocked compatibility status', /blocked_at_compatibility_evaluation/],
    ['unsupported candidate status', /unsupported_candidate/],
    ['invalid pipeline fixture status', /invalid_pipeline_fixture/],
    ...
    ['supported pinned Vault commit', /ab95cbbd24df5817c4e363d24b3b199ac8af6c6f/],
    ['metadata-only pipeline boundary output', /metadata-only local pipeline orchestration/],
    ['alternate Vault execution boundary flag', /alternate_vault_execution:\s*false/],
    ['active Vault switch boundary flag', /active_vault_switch:\s*false/],
    ['multi-Vault runtime support boundary flag', /multi_vault_runtime_support:\s*false/],
    ['candidate eligibility not support acceptance boundary', /candidate_pipeline_eligible_is_support_acceptance:\s*false/]
  ].forEach(([label, pattern]) => {
    if (!pattern.test(text)) {
      addFailure(file, 'Track 3.28 Vault compatibility pipeline', `Script missing ${label}`);
    }
  });
  ...
  prohibitedPatterns.forEach(({ label, pattern }) => {
    if (pattern.test(text)) {
      addFailure(file, 'Track 3.28 Vault compatibility pipeline', `Script contains prohibited ${label}`);
    }
  });
}

function validateTrack329PipelineReportContract(contract) {
  const file = 'data/nexus-vault-compatibility-pipeline-report-contract.v1.json';
  const category = 'Track 3.29 Vault compatibility pipeline report contract';
  ...
  [
    ...
    'allowed_final_pipeline_statuses',
    'required_pipeline_stage_order',
    'blocking_statuses',
    'non_support_statuses',
    'boundary_assertions'
  ].forEach(key => requirePath(contract, [key], file, category));

  const metadata = contract.metadata || {};
  if (metadata.version !== '1.0.0') addFailure(file, category, 'metadata.version must be 1.0.0');
  if (metadata.track_phase !== '3.29') addFailure(file, category, 'metadata.track_phase must be 3.29');
  if (metadata.status !== 'local_pipeline_report_contract') {
    addFailure(file, category, 'metadata.status must be local_pipeline_report_contract');
  }
  [
    ...expectedTrack322BoundaryFalseFlags,
    'alternate_vault_execution',
  ...
  validateExpectedMembers(file, category, contract.allowed_final_pipeline_statuses, expectedTrack328PipelineStatuses, 'allowed_final_pipeline_statuses');
  validateExpectedMembers(file, category, contract.required_pipeline_stage_order, expectedTrack328OrderedStages, 'required_pipeline_stage_order');

  const generated = contract.generated_report || {};
  if (generated.path !== '.track3-runs/latest-nexus-vault-compatibility-pipeline-report.json') {
    addFailure(file, category, 'generated_report.path must point to the ignored pipeline report');
  }
  const validation = contract.validation_report || {};
  if (validation.path !== '.track3-runs/latest-nexus-vault-compatibility-pipeline-report-validation-report.json') {
    addFailure(file, category, 'validation_report.path must point to the ignored validation report');
  }
  [generated, validation].forEach((entry, index) => {
    if (entry.git_ignored !== true || entry.must_not_be_committed !== true || entry.source_of_truth !== false) {
      addFailure(file, category, `${index === 0 ? 'generated_report' : 'validation_report'} must be ignored, uncommitted, and not source_of_truth`);
    }
  });
  ...
  [
    ...
    if (!(contract.required_report_fields || []).includes(field)) {
      addFailure(file, category, `required_report_fields missing ${field}`);
    }
  });

  const boundary = contract.boundary_assertions || {};
  [
    ...
    ['generated_output_is_source_of_truth', false]
  ].forEach(([flag, expected]) => {
    if (boundary[flag] !== expected) addFailure(file, category, `boundary_assertions.${flag} must be ${expected}`);
  });
}

function validateTrack329PipelineReportFixtures(suite, contract) {
  const file = 'data/nexus-vault-compatibility-pipeline-report-fixtures.v1.json';
  const category = 'Track 3.29 Vault compatibility pipeline report fixtures';

  requirePath(suite, ['metadata'], file, category);
  requirePath(suite, ['supported_vault_commit'], file, category);
  requirePath(suite, ['required_fixture_ids'], file, category);
  requirePath(suite, ['report_templates'], file, category);
  requirePath(suite, ['fixtures'], file, category);

  const metadata = suite.metadata || {};
  if (metadata.version !== '1.0.0') addFailure(file, category, 'metadata.version must be 1.0.0');
  if (metadata.track_phase !== '3.29') addFailure(file, category, 'metadata.track_phase must be 3.29');
  if (metadata.status !== 'local_pipeline_report_contract_fixtures') {
    addFailure(file, category, 'metadata.status must be local_pipeline_report_contract_fixtures');
  }
  [
    ...expectedTrack322BoundaryFalseFlags,
    'alternate_vault_execution',
  ...
  if (contract && contract.supported_vault_commit && suite.supported_vault_commit !== contract.supported_vault_commit) {
    addFailure(file, category, 'supported_vault_commit must match report contract');
  }
  validateExpectedMembers(file, category, suite.required_fixture_ids, expectedTrack329ReportFixtureIds, 'required_fixture_ids');
  ...
  fixtures.forEach((fixture, index) => {
    ...
    fixtureIds.add(fixture.fixture_id);
    requirePath(fixture, ['fixture_id'], file, category);
    requirePath(fixture, ['template'], file, category);
    requirePath(fixture, ['expected_valid'], file, category);
    requirePath(fixture, ['mutations'], file, category);
    if (!suite.report_templates || !suite.report_templates[fixture.template]) {
      addFailure(file, category, `${label} references missing template ${fixture.template}`);
    }
    if (typeof fixture.expected_valid !== 'boolean') {
      addFailure(file, category, `${label} expected_valid must be boolean`);
    }
    if (!Array.isArray(fixture.mutations)) {
      addFailure(file, category, `${label} mutations must be an array`);
    }
  });

  expectedTrack329ReportFixtureIds.forEach(fixtureId => {
    if (!fixtureIds.has(fixtureId)) addFailure(file, category, `Missing fixture ${fixtureId}`);
  });
}

function validateTrack329PipelineReportValidatorScript() {
  const file = 'scripts/validate-nexus-vault-compatibility-pipeline-report.mjs';
  if (!existsSync(path.join(repoRoot, file))) {
    addFailure(file, 'Track 3.29 pipeline report validator', 'Pipeline report validator script is missing');
    return;
  }
  ...
  [
    ...
    ['runtime report input', /\.track3-runs\/latest-nexus-vault-compatibility-pipeline-report\.json/],
    ['ignored validation output', /\.track3-runs\/latest-nexus-vault-compatibility-pipeline-report-validation-report\.json/],
    ['current supported pass status', /current_supported_pass/],
    ['candidate pipeline eligible status', /candidate_pipeline_eligible/],
    ['unsupported candidate status', /unsupported_candidate/],
    ['packet validation stage', /packet_validation/],
    ['candidate intake stage', /candidate_intake/],
    ['compatibility evaluation stage', /compatibility_evaluation/],
    ['final pipeline status stage', /final_pipeline_status/],
    ['supported pinned Vault commit', /ab95cbbd24df5817c4e363d24b3b199ac8af6c6f/],
    ['shape boundary output', /report shape and boundary assertion validation only/],
    ['alternate Vault execution boundary flag', /alternate_vault_execution:\s*false/],
    ['active Vault switch boundary flag', /active_vault_switch:\s*false/],
    ['multi-Vault runtime support boundary flag', /multi_vault_runtime_support:\s*false/]
  ].forEach(([label, pattern]) => {
    if (!pattern.test(text)) addFailure(file, 'Track 3.29 pipeline report validator', `Script missing ${label}`);
  });
  ...
  prohibitedPatterns.forEach(({ label, pattern }) => {
    if (pattern.test(text)) addFailure(file, 'Track 3.29 pipeline report validator', `Script contains prohibited ${label}`);
  });
}
...
function validateTrack331FailureInjectionReportContract(contract) {
  const file = 'data/nexus-vault-compatibility-pipeline-failure-injection-report-contract.v1.json';
  const category = 'Track 3.31 Vault compatibility pipeline failure-injection report contract';
  ...
  [
    ...
    'validation_report',
    'required_report_fields',
    'required_per_injection_fields',
    'allowed_injection_categories',
    'allowed_expected_outcomes',
    'allowed_actual_outcomes',
    'passing_actual_outcomes',
    'failing_actual_outcomes',
    'boundary_assertions'
  ].forEach(key => requirePath(contract, [key], file, category));

  const metadata = contract.metadata || {};
  if (metadata.version !== '1.0.0') addFailure(file, category, 'metadata.version must be 1.0.0');
  if (metadata.track_phase !== '3.31') addFailure(file, category, 'metadata.track_phase must be 3.31');
  if (metadata.status !== 'local_pipeline_failure_injection_report_contract') {
    addFailure(file, category, 'metadata.status must be local_pipeline_failure_injection_report_contract');
  }
  [
    ...expectedTrack322BoundaryFalseFlags,
    'alternate_vault_execution',
  ...
  if (contract.report_version !== '1.0.0') addFailure(file, category, 'report_version must be 1.0.0');
  if (contract.suite_version !== '1.0.0') addFailure(file, category, 'suite_version must be 1.0.0');
  if (contract.supported_vault_commit !== 'ab95cbbd24df5817c4e363d24b3b199ac8af6c6f') {
    addFailure(file, category, 'supported_vault_commit must match accepted pinned Vault commit');
  }
  validateExpectedMembers(file, category, contract.allowed_injection_categories, expectedTrack330FailureInjectionCategories, 'allowed_injection_categories');
  validateExpectedMembers(file, category, contract.allowed_expected_outcomes, expectedTrack330FailureInjectionOutcomes, 'allowed_expected_outcomes');
  validateExpectedMembers(file, category, contract.allowed_actual_outcomes, expectedTrack330FailureInjectionOutcomes, 'allowed_actual_outcomes');

  const generated = contract.generated_report || {};
  if (generated.path !== '.track3-runs/latest-nexus-vault-compatibility-pipeline-failure-injection-report.json') {
    addFailure(file, category, 'generated_report.path must point to the ignored failure-injection report');
  }
  const validation = contract.validation_report || {};
  if (validation.path !== '.track3-runs/latest-nexus-vault-compatibility-pipeline-failure-injection-report-validation-report.json') {
    addFailure(file, category, 'validation_report.path must point to the ignored validation report');
  }
  [generated, validation].forEach((entry, index) => {
    if (entry.git_ignored !== true || entry.must_not_be_committed !== true || entry.source_of_truth !== false) {
  ...
  [
    ...
    if (!(contract.required_per_injection_fields || []).includes(field)) {
      addFailure(file, category, `required_per_injection_fields missing ${field}`);
    }
  });
  ...
  [
    ...
    ['candidate_pipeline_eligible_is_support_acceptance', false],
    ['generated_track3_runs_reports_committed', false],
...
function validateTrack331FailureInjectionReportFixtures(suite, contract) {
  const file = 'data/nexus-vault-compatibility-pipeline-failure-injection-report-fixtures.v1.json';
  const category = 'Track 3.31 Vault compatibility pipeline failure-injection report fixtures';
  ...
  if (metadata.version !== '1.0.0') addFailure(file, category, 'metadata.version must be 1.0.0');
  if (metadata.track_phase !== '3.31') addFailure(file, category, 'metadata.track_phase must be 3.31');
  if (metadata.status !== 'local_pipeline_failure_injection_report_contract_fixtures') {
    addFailure(file, category, 'metadata.status must be local_pipeline_failure_injection_report_contract_fixtures');
  }
  [
    ...expectedTrack322BoundaryFalseFlags,
    'alternate_vault_execution',
  ...
  validateExpectedMembers(file, category, suite.required_fixture_ids, expectedTrack331FailureInjectionReportFixtureIds, 'required_fixture_ids');
  ...
  expectedTrack331FailureInjectionReportFixtureIds.forEach(fixtureId => {
    if (!fixtureIds.has(fixtureId)) addFailure(file, category, `Missing fixture ${fixtureId}`);
  });
}

function validateTrack331FailureInjectionReportValidatorScript() {
  const file = 'scripts/validate-nexus-vault-compatibility-pipeline-failure-injection-report.mjs';
  if (!existsSync(path.join(repoRoot, file))) {
    addFailure(file, 'Track 3.31 failure-injection report validator', 'Failure-injection report validator script is missing');
    return;
  }

  const text = readText(file);
  [
    ['report contract input', /data\/nexus-vault-compatibility-pipeline-failure-injection-report-contract\.v1\.json/],
    ['report fixtures input', /data\/nexus-vault-compatibility-pipeline-failure-injection-report-fixtures\.v1\.json/],
    ['runtime report input', /\.track3-runs\/latest-nexus-vault-compatibility-pipeline-failure-injection-report\.json/],
    ['ignored validation output', /\.track3-runs\/latest-nexus-vault-compatibility-pipeline-failure-injection-report-validation-report\.json/],
    ['blocked outcome', /injected_failure_blocked/],
    ['rejected outcome', /injected_failure_rejected/],
    ['expectation failed outcome', /injection_expectation_failed/],
    ['failure injection category', /non_pinned_candidate_marked_current_supported/],
    ['supported pinned Vault commit', /ab95cbbd24df5817c4e363d24b3b199ac8af6c6f/],
    ['shape boundary output', /failure-injection report shape and boundary assertion validation only/],
    ['alternate Vault execution boundary flag', /alternate_vault_execution:\s*false/],
    ['active Vault switch boundary flag', /active_vault_switch:\s*false/],
    ['multi-Vault runtime support boundary flag', /multi_vault_runtime_support:\s*false/]
  ].forEach(([label, pattern]) => {
    if (!pattern.test(text)) addFailure(file, 'Track 3.31 failure-injection report validator', `Script missing ${label}`);
  });
  ...
  prohibitedPatterns.forEach(({ label, pattern }) => {
    if (pattern.test(text)) addFailure(file, 'Track 3.31 failure-injection report validator', `Script contains prohibited ${label}`);
  });
}
...
if (nexusVaultCompatibilityPipelineFixtures) {
  validateTrack328CompatibilityPipelineFixtures(nexusVaultCompatibilityPipelineFixtures, nexusVaultEvidencePacketContract);
}
if (nexusVaultCompatibilityPipelineReportContract) {
  validateTrack329PipelineReportContract(nexusVaultCompatibilityPipelineReportContract);
}
...
if (nexusVaultCompatibilityPipelineFailureInjectionReportContract) {
  validateTrack331FailureInjectionReportContract(nexusVaultCompatibilityPipelineFailureInjectionReportContract);
}
if (nexusVaultCompatibilityPipelineFailureInjectionReportFixtures) {
  validateTrack331FailureInjectionReportFixtures(nexusVaultCompatibilityPipelineFailureInjectionReportFixtures, nexusVaultCompatibilityPipelineFailureInjectionReportContract);
}
...
validateTrack331FailureInjectionReportValidatorScript();
